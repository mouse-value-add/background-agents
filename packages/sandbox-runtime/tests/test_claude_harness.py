...
class TestSession:
    ...
    async def test_fresh_session_gets_a_new_id_and_connects_with_session_id(self, tmp_path: Path):
        ...
        await h.harness.open()
        session_id = await h.harness.create_or_resume_session(None)
        assert session_id == h.harness.session_id
    ...
    async def test_persisted_session_without_a_transcript_starts_fresh(self, tmp_path: Path):
        h = Harness(tmp_path)
        await h.harness.open()
        assert await h.harness.create_or_resume_session("gone") != "gone"
...
class TestOptions:
    ...
    async def test_options_follow_the_design_mapping(self, tmp_path: Path) -> None:
        ...
        await h.harness.open()
        await h.harness.create_or_resume_session(None)
        await _run(
            h.harness,
            HarnessPrompt(
                message_id="m1",
